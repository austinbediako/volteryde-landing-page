...
export interface CardNavProps {
	logo: any; // Allow StaticImageData from next/image or string
	logoAlt?: string;
	items: CardNavItem[];
	className?: string;
	ease?: string;
...
const CardNav: React.FC<CardNavProps> = ({
	...
	return (
		...
		>
			<nav
				ref={navRef}
				className={`card-nav ${isExpanded ? 'open' : ''} block h-[60px] p-0 rounded-xl shadow-md relative overflow-hidden will-change-[height] bg-white`}
			...
			>
				<div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
					...
					<div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 order-1 md:order-none">
						{/* Added Next.js Image support or generic img */}
						{typeof logo === 'string' ? (
							<img src={logo} alt={logoAlt} className="logo h-[28px] w-auto" />
						) : (
							<Image src={logo} alt={logoAlt} className="logo h-[28px] w-auto object-contain" />
						)}
					</div>

					<a
						href={ctaHref}
						className="card-nav-cta-button hidden md:inline-flex border-0 rounded-[calc(0.75rem-0.2rem)] px-4 items-center h-full font-medium cursor-pointer transition-colors duration-300 flex justify-center no-underline bg-[#044805] text-white"
